...
if ($PSBoundParameters.ContainsKey('FileName')) {
    ...
    $Writer.TypeParagraph()
    $Writer.Style = 'Normal'
    $Writer.TypeText("This report describes the current situation to adopt Sentinel in Defender in terms of Table Retention, Analytics Rules and Automations Rules. The report analyzes Sentinel environments, ")
...
foreach ($env in $environments) {
    ...
    foreach ($rule in $response.value) {
        ...
        ### CUSTOM DETAILS OVERRIDE ANALYSIS
        Write-Host "- Custom Details Override Analysis"
        if ($PSBoundParameters.ContainsKey('FileName')) {
            Set-WriterStyle -Writer $Writer -Bold $true
            $Writer.TypeText("Custom Details Override Analysis")
            Set-WriterStyle -Writer $Writer -Bold $false
            $Writer.TypeText([char]11)
        }
        if ($rule.properties.customDetails) {
            foreach ($detail in ($rule.properties.customDetails | Get-Member -MemberType NoteProperty).Name) {
                Write-Host "  [OK]" -ForegroundColor Green -NoNewline; Write-Host " Found a custom detail override ($detail) - Custom Detection Rules supports it"
                if ($PSBoundParameters.ContainsKey('FileName')) {
                    Set-WriterStyle -Writer $Writer -Color 5287936 -Bold $true
                    $Writer.TypeText("[OK] ")
                    Set-WriterStyle -Writer $Writer -Bold $false -Color 0
                    $Writer.TypeText("Found a custom detail override ($detail) - Custom Detection Rules supports it")
                    $Writer.TypeText([char]11)
                }
            }
        }
        else {
            Write-Host "  [OK]" -ForegroundColor Green -NoNewline; Write-Host " No custom details override defined"
            if ($PSBoundParameters.ContainsKey('FileName')) {
                Set-WriterStyle -Writer $Writer -Color 5287936 -Bold $true
                $Writer.TypeText("[OK] ")
                Set-WriterStyle -Writer $Writer -Bold $false -Color 0
                $Writer.TypeText("No custom details override defined")
                $Writer.TypeText([char]11)
            }
        }
